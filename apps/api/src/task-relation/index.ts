...
// ASYGNUZ: bulk variant for the list view's Épica -> Tareas -> Subtareas
// tree -- one call per project instead of one getTaskRelations call per task.
const getProjectSubtaskRelationsRoute = createRoute({
  method: "get",
  operationId: "getProjectSubtaskRelations",
  path: "/project/{projectId}",
  tags: ["Task Relations"],
  summary: "Get project subtask relations",
  description:
    "Get every subtask relation whose parent task lives in this project, in one call (ASYGNUZ, for rendering the task list as a tree).",
  middleware: [workspaceAccess.fromProject("projectId")] as const,
  request: { params: projectIdParam },
  responses: {
    200: jsonResponse(
      "Subtask relations for the project",
      projectSubtaskRelationListSchema,
    ),
    400: errorResponse(
