...
const project = apiRouter<BaseVariables & { workspaceId: string }>()
  .openapi(listProjectsRoute, async (c) => {
    ...
    // ASYGNUZ: needed so getProjectsCtrl can filter out projects restricted
    // via project_member. Upstream's own version of this route doesn't pass
    // it — losing it here would silently un-filter the list on the next sync.
